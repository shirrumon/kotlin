...
// External implementations for intrinsics that atomically update array elements:

// __atomic_load_n the element at the given index of the IntArray located at *location.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW KInt AtomicGetIntArrayElement(ObjHeader* location, KInt index) {
    KInt* arrayElement = IntArrayAddressOfElementAt(location->array(), index);
    return mm::GetIntFieldSeqCst(arrayElement);
}

// __atomic_store_n the element at the given index of the IntArray located at *location.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW void AtomicSetIntArrayElement(ObjHeader* location, KInt index, KInt newValue) {
    KInt* arrayElement = IntArrayAddressOfElementAt(location->array(), index);
    mm::SetIntFieldSeqCst(arrayElement, newValue);
}

// __atomic_exchange_n the element at the given index of the IntArray located at *location with the newValue
// and return the old value of the element.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW KInt GetAndSetIntArrayElement(ObjHeader* location, KInt index, KInt newValue) {
    KInt* arrayElement = IntArrayAddressOfElementAt(location->array(), index);
    return mm::GetAndSetIntField(arrayElement, newValue);
}

// __atomic_fetch_add delta to the element at the given index of the IntArray located at *location
// and return the old value of the element.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW KInt GetAndAddIntArrayElement(ObjHeader* location, KInt index, KInt delta) {
    KInt* arrayElement = IntArrayAddressOfElementAt(location->array(), index);
    return mm::FetchAndAddIntField(arrayElement, delta);
}

// __atomic_compare_exchange_n the element at the given index of the IntArray located at *location from the expectValue to the newValue
// and return the old value of the element.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW KInt CompareAndExchangeIntArrayElement(ObjHeader* location, KInt index, KInt expectValue, KInt newValue) {
    KInt* arrayElement = IntArrayAddressOfElementAt(location->array(), index);
    return mm::CompareAndExchangeIntField(arrayElement, &expectValue, newValue);
}

// __atomic_compare_exchange_n the element at the given index of the IntArray located at *location from the expectValue to the newValue
// and return whether the operation was successful.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW bool CompareAndSetIntArrayElement(ObjHeader* location, KInt index, KInt expectValue, KInt newValue) {
    KInt* arrayElement = IntArrayAddressOfElementAt(location->array(), index);
    return mm::CompareAndSetIntField(arrayElement, &expectValue, newValue);
}

// __atomic_load_n the element at the given index of the LongArray located at *location.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW KLong AtomicGetLongArrayElement(ObjHeader* location, KInt index) {
    KLong* arrayElement = LongArrayAddressOfElementAt(location->array(), index);
    return mm::GetLongFieldSeqCst(arrayElement);
}

// __atomic_store_n the element at the given index of the LongArray located at *location.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW void AtomicSetLongArrayElement(ObjHeader* location, KInt index, KLong newValue) {
    KLong* arrayElement = LongArrayAddressOfElementAt(location->array(), index);
    mm::SetLongFieldSeqCst(arrayElement, newValue);
}

// __atomic_exchange_n the element at the given index of the LongArray located at *location with the newValue
// and return the old value of the element.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW KLong GetAndSetLongArrayElement(ObjHeader* location, KInt index, KLong newValue) {
    KLong* arrayElement = LongArrayAddressOfElementAt(location->array(), index);
    return mm::GetAndSetLongField(arrayElement, newValue);
}

// __atomic_fetch_add delta to the element at the given index of the LongArray located at *location
// and return the old value of the element.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW KLong GetAndAddLongArrayElement(ObjHeader* location, KInt index, KLong delta) {
    KLong* arrayElement = LongArrayAddressOfElementAt(location->array(), index);
    return mm::FetchAndAddLongField(arrayElement, delta);
}

// __atomic_compare_exchange_n the element at the given index of the LongArray located at *location from the expectValue to the newValue
// and return the old value of the element.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW KLong CompareAndExchangeLongArrayElement(ObjHeader* location, KInt index, KLong expectValue, KLong newValue) {
    KLong* arrayElement = LongArrayAddressOfElementAt(location->array(), index);
    return mm::CompareAndExchangeLongField(arrayElement, &expectValue, newValue);
}

// __atomic_compare_exchange_n the element at the given index of the LongArray located at *location from the expectValue to the newValue
// and return whether the operation was successful.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW bool CompareAndSetLongArrayElement(ObjHeader* location, KInt index, KLong expectValue, KLong newValue) {
    KLong* arrayElement = LongArrayAddressOfElementAt(location->array(), index);
    return mm::CompareAndSetLongField(arrayElement, &expectValue, newValue);
}

// __atomic_load_n the element at the given index of the Array<T> located at *location.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW OBJ_GETTER(AtomicGetArrayElement, ObjHeader* location, KInt index) {
    KRef* arrayElement = ArrayAddressOfElementAt(location->array(), index);
    RETURN_RESULT_OF(mm::GetHeapRefSeqCst, arrayElement);
}

// __atomic_store_n the element at the given index of the Array<T> located at *location.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW void AtomicSetArrayElement (ObjHeader* location, KInt index, ObjHeader* newValue) {
    KRef* arrayElement = ArrayAddressOfElementAt(location->array(), index);
    mm::SetHeapRefAtomicSeqCst(arrayElement, newValue);
}

// __atomic_exchange_n the element at the given index of the Array<T> located at *location with the newValue
// and return the old value of the element.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW OBJ_GETTER(GetAndSetArrayElement, ObjHeader* location, KInt index, ObjHeader* newValue) {
    KRef* arrayElement = ArrayAddressOfElementAt(location->array(), index);
    RETURN_RESULT_OF(GetAndSetVolatileHeapRef, arrayElement, newValue);
}

// __atomic_compare_exchange_n the element at the given index of the Array<T> located at *location from the expectValue to the newValue
// and return the old value of the element.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW OBJ_GETTER(CompareAndExchangeArrayElement, ObjHeader* location, KInt index, ObjHeader* expectValue, ObjHeader* newValue) {
    KRef* arrayElement = ArrayAddressOfElementAt(location->array(), index);
    RETURN_RESULT_OF(mm::CompareAndSwapHeapRef, arrayElement, expectValue, newValue);
}

// __atomic_compare_exchange_n the element at the given index of the Array<T> located at *location from the expectValue to the newValue
// and return whether the operation was successful.
extern "C" ALWAYS_INLINE RUNTIME_NOTHROW bool CompareAndSetArrayElement(ObjHeader* location, KInt index, ObjHeader* expectValue, ObjHeader* newValue) {
    KRef* arrayElement = ArrayAddressOfElementAt(location->array(), index);
    return mm::CompareAndSetHeapRef(arrayElement, expectValue, newValue);
}
