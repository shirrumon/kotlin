...
object AnnotationsCommonizer : AssociativeCommonizer<List<CirAnnotation>> {
    ...
    private fun commonizedObjcCallableAnnotation(first: List<CirAnnotation>, second: List<CirAnnotation>): List<CirAnnotation> {
        val firstObjCCallable = first.filter { it.type.classifierId.isObjCInteropCallableAnnotation }
        val secondObjCCallable = second.filter { it.type.classifierId.isObjCInteropCallableAnnotation }
        return buildList {
            if (firstObjCCallable.isNotEmpty() && secondObjCCallable.isNotEmpty()) {
                add(objCCallableAnnotation)
                for (first in firstObjCCallable) {
                    for (second in secondObjCCallable) {
                            ...
                            val annotation = CirAnnotation.createInterned(
                                ...
                                buildMap {
                                    for ((key, value) in first.constantValueArguments) {
                                        ...
                                        when (key.name) {
                                            "selector", "initSelector" -> continue
