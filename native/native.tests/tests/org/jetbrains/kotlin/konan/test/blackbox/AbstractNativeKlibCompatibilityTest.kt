...
import org.jetbrains.kotlin.klib.KlibCompatibilityCompilerVersions
import org.jetbrains.kotlin.klib.KlibCompatibilityCompilerVersionsScenario
import org.jetbrains.kotlin.klib.KlibCompatibilityTestUtils
import org.jetbrains.kotlin.klib.KlibCompatibilityTestUtils.Dependency
import org.jetbrains.kotlin.klib.KlibCompatibilityTestUtils.Dependencies
import org.jetbrains.kotlin.klib.KlibCompatibilityTestUtils.MAIN_MODULE_NAME
...
abstract class AbstractNativeKlibCompatibilityTest : AbstractNativeSimpleTest() {
    private inner class NativeTestConfiguration(testPath: String) : KlibCompatibilityTestUtils.TestConfiguration {
        override val testDir = getAbsoluteFile(testPath)
        override val buildDir get() = this@AbstractNativeKlibCompatibilityTest.buildDir
        override val stdlibFile get() = this@AbstractNativeKlibCompatibilityTest.stdlibFile
        ...
        override fun buildKlib(
            moduleName: String,
            buildDirs: KlibCompatibilityTestUtils.ModuleBuildDirs,
            dependencies: KlibCompatibilityTestUtils.Dependencies,
            klibFile: File,
            compilerVersion: KlibCompatibilityCompilerVersions,
        ) = this@AbstractNativeKlibCompatibilityTest.buildKlib(moduleName, buildDirs.sourceDir, dependencies, klibFile, compilerVersion)

        override fun buildBinaryAndRun(
            mainModule: KlibCompatibilityTestUtils.Dependency,
            otherDependencies: KlibCompatibilityTestUtils.Dependencies,
        ) =
            this@AbstractNativeKlibCompatibilityTest.buildBinaryAndRun(mainModule, otherDependencies)
    ...
    private class ProducedKlib(val moduleName: String, val klibArtifact: KLIB, val dependencies: KlibCompatibilityTestUtils.Dependencies) {
    ...
    private val executableArtifact: Executable by lazy {
        val (_, outputDir) = KlibCompatibilityTestUtils.createModuleDirs(buildDir, LAUNCHER_MODULE_NAME)
    ...
    protected fun runTest(@TestDataFile testPath: String) =
        KlibCompatibilityCompilerVersionsScenario.entries.forEach { compilerVersionsSwitch ->
            KlibCompatibilityTestUtils.runTest(NativeTestConfiguration(testPath), compilerVersionsSwitch)
        }
    ...
    private fun buildKlib(
        ...
        when (compilerVersion) {
            KlibCompatibilityCompilerVersions.CURRENT -> buildKlibCurrent(moduleName, moduleSourceDir, dependencies, klibFile)
            else -> buildKlibReleased(moduleName, moduleSourceDir, dependencies, klibFile, compilerVersion.value)
        }
    }
    ...
    private fun createTestCase(moduleName: String, moduleSourceDir: File?, compilerArgs: TestCompilerArgs): TestCase {
        ...
        val module: TestModule.Exclusive? = moduleSourceDir?.let {
            TestModule.Exclusive(
                ...
                /* Don't need to pass any dependency symbols here.
    ...
    private fun createIncludedDependency(dependency: KlibCompatibilityTestUtils.Dependency): TestCompilationDependency<KLIB> =
