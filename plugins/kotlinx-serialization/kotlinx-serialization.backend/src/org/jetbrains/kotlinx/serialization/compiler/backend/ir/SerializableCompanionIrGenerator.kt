...
class SerializableCompanionIrGenerator(
    ...
    fun generate() {
        val serializerGetterFunction =
            getSerializerGetterFunction(serializableIrClass)?.takeIf {
                it.isFromPlugin(
        ...
        if (serializableIrClass.hasKeepGeneratedSerializerAnnotation) {
            val keepSerializerGetterFunction =
                getSerializerGetterFunction(serializableIrClass, true)?.takeIf { it.isFromPlugin(compilerContext.afterK2) }
                    ?: throw IllegalStateException(
