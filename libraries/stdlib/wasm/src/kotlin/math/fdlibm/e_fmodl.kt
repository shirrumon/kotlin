...
private fun Zero(i: UInt): Double = if (i == 0U) 0.0 else -0.0

internal fun __ieee754_fmodl(_x: Double, _y: Double): Double {
    var x = _x
    val y = _y
    ...
    if ((hy or ly.toInt()) == 0 || (hx >= 0x7ff00000) ||    /* y=0,or x not finite */
        ((hy or ((ly or (-ly.toInt()).toUInt()) shr 31).toInt()) > 0x7ff00000)
    )    /* or y is NaN */
        return (x * y) / (x * y)
    if (hx <= hy) {
        if ((hx < hy) || (lx < ly))
        ...
        if (lx == ly)
            return Zero(sx.toUInt() shr 31)    /* |x|=|y| return x*0 */
    }

    /* determine ix = ilogb(x) */
    if (hx < 0x00100000) {                                    /* subnormal x */
        if (hx == 0) {
            //for (ix = -1043, i = lx; i > 0; i shl= 1)
            ix = -1043
            i = lx.toInt()
            while (i > 0) {
                ix -= 1
            ...
            //for (ix = -1022, i = (hx shl 11); i > 0; i shl= 1)
            ix = -1022
            i = (hx shl 11)
            while (i > 0) {
                ix -= 1
    ...
    if (hy < 0x00100000) {                                    /* subnormal y */
        if (hy == 0) {
            ...
            //for (iy = -1022, i = (hy shl 11); i > 0; i shl= 1)
            iy = -1022
            i = (hy shl 11)
            while (i > 0) {
                iy -= 1
    ...
    if (iy >= -1022) {                                    /* normalize output */
        ...
        n = -1022 - iy
        if (n <= 20) {
            lx = (lx shr n) or (hx shl (32 - n)).toUInt()
            hx = hx ushr n
        } else if (n <= 31) {
