...
class Bir2IrConverter(
    ...
    private fun copyErrorDeclaration(old: BirErrorDeclaration): IrErrorDeclaration = copyNotReferencedElement(old, {
        IrErrorDeclarationImpl(
            startOffset = old.sourceSpan.start,
            endOffset = old.sourceSpan.end,
            old[GlobalBirElementDynamicProperties.Descriptor],
        )
    }) {
        origin = old.origin
        copyDynamicProperties(old)
    ...
    private fun IrElement.copyDynamicProperties(
        ...
        if (this is IrMetadataSourceOwner) {
            metadata = (from as BirMetadataSourceOwner)[GlobalBirElementDynamicProperties.Metadata]
        }

        if (this is IrMemberWithContainerSource) {
            val containerSource = (from as BirMemberWithContainerSource)[GlobalBirElementDynamicProperties.ContainerSource]
            when (this) {
        ...
        if (this is IrAttributeContainer) {
            originalBeforeInline = (from as BirAttributeContainer)[GlobalBirElementDynamicProperties.OriginalBeforeInline]
                ?.let { remapElement(it) as IrAttributeContainer }
        }

        if (this is IrClass) {
            sealedSubclasses = (from as BirClass)[GlobalBirElementDynamicProperties.SealedSubclasses]
