...
expect <!WRONG_MODIFIER_TARGET{JVM}!>external<!> var prop: String

expect var getAndSet: String
    external get
    external set

<!WRONG_MODIFIER_TARGET{JVM}!>external<!> expect val explicitGetter: String
    external get

expect <!WRONG_MODIFIER_TARGET{JVM}!>external<!> class A {
    external fun foo()
    fun bar()
}
