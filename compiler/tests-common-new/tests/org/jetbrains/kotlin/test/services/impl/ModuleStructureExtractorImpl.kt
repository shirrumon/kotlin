...
class ModuleStructureExtractorImpl(
    ...
    override fun splitTestDataSourceCodeByModules(
        ...
        val extractor = MinimalModuleStructureExtractorWorker(listOf(testDataSourceCode), directivesContainer)
        val fakeSourceFile = File("fake_source_file.kt")
        extractor.currentTestDataFile = fakeSourceFile
        val sortedModules = extractor.splitTestDataSourcesByModules()
//        var index = 0
        var result: TestModuleStructure = TestModuleStructureImpl(
            sortedModules,
            // This "file" will only be used as the key
            // to store meta infos for the modules.
            listOf(fakeSourceFile),
//            sortedModules.map { File("file_${index++}.kt") },
        )
        for (transformer in moduleStructureTransformers) {
            result = try {
    ...
    private inner class MinimalModuleStructureExtractorWorker constructor(
        private val testDataFilesSources: List<String>,
        private val directivesContainer: DirectivesContainer,
    ) {
        private val assertions: Assertions
            get() = testServices.assertions
        ...
        lateinit var currentTestDataFile: File
        ...
        fun splitTestDataSourcesByModules(): List<TestModule> {
            for (testDataFileSource in testDataFilesSources) {
                val lines = testDataFileSource.split("\n")
                lines.forEachIndexed { lineNumber, line ->
                    val rawDirective = RegisteredDirectivesParser.parseDirective(line)
                    if (tryParseStructureDirective(rawDirective, lineNumber + 1)) {
                        linesOfCurrentFile.add(line)
