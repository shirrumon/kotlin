...
class ModuleStructureExtractorImpl(
    ...
    override fun splitTestDataByModules(
        ...
        val extractor = ModuleStructureExtractorWorker(directivesContainer)
        return splitTestDataSourceCodeByModules(extractor.splitFilesTestDataByModules(listOf(testDataFile)))
    }

    override fun splitTestDataSourceCodeByModules(
        testDataSourceCode: String,
        directivesContainer: DirectivesContainer,
    ): TestModuleStructure {
        val extractor = ModuleStructureExtractorWorker(directivesContainer)
        return splitTestDataSourceCodeByModules(extractor.splitRawSourceTestDataByModules(testDataSourceCode))
    }

    private fun splitTestDataSourceCodeByModules(
        testModuleStructure: TestModuleStructure,
    ): TestModuleStructure {
        var result = testModuleStructure
    ...
    private inner class ModuleStructureExtractorWorker(
        ...
        private var currentTestDataFile: File? = null

        private val defaultFileName: String
            get() = currentTestDataFile?.name ?: "Default.kt"
        ...
        fun splitFilesTestDataByModules(realTestDataFiles: List<File>): TestModuleStructure {
            for (testDataFile in realTestDataFiles) {
                currentTestDataFile = testDataFile
                splitSourceLinesByModules(testDataFile.readLines())
            }

            return TestModuleStructureImpl(finalizeModules(), realTestDataFiles)
        }

        fun splitRawSourceTestDataByModules(source: String): TestModuleStructure {
            splitSourceLinesByModules(source.split("\n"))
            return TestModuleStructureImpl(finalizeModules(), emptyList())
        }

        private fun finalizeModules(): List<TestModule> {
        ...
        private fun splitSourceLinesByModules(lines: List<String>) {
