...
class LLSessionProvider(
    ...
    // The `LLFirSession` must be strongly reachable from the resolvable session and ultimately the `KtFirAnalysisSession` because soft
    // reference garbage collection should collect `KtFirAnalysisSession` and its `LLFirSession` together or not at all. See `LLFirSession`
    // for more details.
